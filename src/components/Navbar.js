...
function Navbar() {
    ...
    const [isNavbarActive, setIsNavbarActibe] = useState(false);

    const handleNavberActive = () => {
        if(window.scrollY >= 1)
        {
            setIsNavbarActibe(true)
        }
        else {
            setIsNavbarActibe(false)
        }
    }
    
    useEffect(() => {
        window.addEventListener('scroll', handleNavberActive);
    
        return () => {
          window.removeEventListener('scroll', handleNavberActive);
        };
      }, []);
